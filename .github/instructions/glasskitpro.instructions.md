...
### GlassKitPro — Dependencia de Diseño Obligatoria
```yaml
INFORMACIÓN:
- Package: GlassKitPro v1.0.3+
- URL: https://github.com/MoLinesGitHub/GlassKitPro.git
- Namespace: GlassKit
- Documentación: .github/instructions/glasskitpro.instructions.md
...
- GlassKit.CrystalParticles       → Fondos animados
- GlassKit.CrystalLiquidCard      → Tarjetas líquidas
- GlassKit.AdvancedGlassCard      → Tarjetas glass
- GlassKit.AdvancedGlassButton    → Botones cristal
- GlassKit.CrystalFloatingActionButton → FABs
- GlassKit.CrystalMetricCard      → Métricas

PARÁMETROS RECOMENDADOS PARA CrystalParticles:
- particleCount: 15-25 (máximo 30)
- opacity: 0.10-0.20 (máximo 0.25)
- speedMultiplier: 0.3-0.5 (máximo 0.6)
- baseColor: usar paleta Anstop (cyan, green, purple, indigo)

COLORES POR HERRAMIENTA:
- Respiración: .cyan (tranquilidad)
- Grounding: .green (equilibrio)
- Audio: .purple (espiritualidad)
- Diario: .indigo (introspección)
- Biblioteca: .teal (claridad)
- AI Helper: Color(0.6, 0.4, 0.8) (compasión)
- Historial: .orange (calidez)

PROHIBICIONES:
- ❌ Componentes sin namespace GlassKit.
- ❌ opacity > 0.30 (distrae al usuario)
- ❌ speedMultiplier > 0.7 (movimiento estresante)
- ❌ Colores agresivos: .red, amarillo brillante
- ❌ particleCount > 50 (afecta performance)
- ❌ Partículas en cada celda de lista (solo en fondos)

VALIDACIÓN AUTOMÁTICA:
Al detectar cambios en UI/, verificar:
1. ¿Vista principal tiene import GlassKitPro?
2. ¿Usa namespace GlassKit. correcto?
3. ¿Parámetros de CrystalParticles dentro de límites?
4. ¿Color es de la paleta Anstop?
```
