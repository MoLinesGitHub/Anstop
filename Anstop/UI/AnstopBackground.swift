...
private struct SimpleParticlesView: View {
            ...
            TimelineView(.animation(minimumInterval: 1/30)) { timeline in
                Canvas { context, size in
                    ...
                    for particle in particles {
                        ...
                        var offsetX = (particle.xPosition + particle.speedX * timeOffset * 30).truncatingRemainder(dividingBy: size.width)
                        var offsetY = (particle.yPosition + particle.speedY * timeOffset * 30).truncatingRemainder(dividingBy: size.height)

                        if offsetX < 0 { offsetX += size.width }
                        if offsetY < 0 { offsetY += size.height }

                        let rect = CGRect(x: offsetX, y: offsetY, width: particle.size, height: particle.size)
                        context.fill(
                            Path(ellipseIn: rect),
    ...
    private func initializeParticles(in size: CGSize) {
        ...
        particles = (0..<count).map { _ in
            Particle(
                xPosition: CGFloat.random(in: 1...size.width),
                yPosition: CGFloat.random(in: 1...size.height),
...
private struct LiquidWavesView: View {
    ...
    private func drawWaves(context: GraphicsContext, size: CGSize, phase: Double) {
        ...
        for xCoord in stride(from: 0, through: size.width, by: 2) {
            let relativeX = xCoord / size.width
            let sine = sin(relativeX * 6 + phase)
            let yCoord = size.height * 0.6 + sine * 20 * intensity
            path1.addLine(to: CGPoint(x: xCoord, y: yCoord))
        }
        ...
        for xCoord in stride(from: 0, through: size.width, by: 2) {
            ...
            let yCoord = size.height * 0.7 + sine * 30 * intensity
            path2.addLine(to: CGPoint(x: xCoord, y: yCoord))
        }
