    ...
    try {
        switch (name) {
            ...
            case 'anstop_build_project': {
                ...
                const errorLines = result.split('\n').filter(line => line.includes('error:') || line.includes('Error:') || line.includes('failed')).slice(0, 15);
                // Análisis de errores de concurrency
                const concurrencyErrors = errorLines.filter(line => line.includes('@MainActor') ||
