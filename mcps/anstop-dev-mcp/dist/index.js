...
class AnstopDevMCPServer {
     ...
     */
    async analyzeSwiftCompilationErrors(args) {
    ...
    async validateArchitecture(args) {
        const { check_layer_separation = true, check_dependencies = true, check_concurrency_rules = true, validate_domain_purity = true } = args;
     ...
     */
    async runTests(args) {
        const { test_target = 'all', test_class, test_method, generate_coverage = false, analyze_failures = true } = args;
     ...
     */
    async analyzeXcodeBuildLogs(args) {
    ...
    async optimizePerformance(args) {
        const { layer = 'all', check_swiftui_performance = true, check_animations = true, check_memory = true, suggest_instruments = true } = args;
     ...
     */
    async validateLocalization(args) {
        const { check_missing_keys = true, check_hardcoded_strings = true, languages = ['es', 'en'] } = args;
     ...
     */
    async checkAssets(args) {
        const { check_colors = true, check_images = true, check_dark_mode = true, check_accessibility = true } = args;
    ...
    parseSwiftErrors(logContent, includeWarnings) {
        const errors = [];
    ...
    generateErrorAnalysis(errors) {
        ...
        const concurrencyErrors = errors.filter(e => e.line.includes('@MainActor') || e.line.includes('Sendable') || e.line.includes('actor'));
    ...
    analyzeBuildLog(logContent, filterLevel) {
        return {
    ...
    generateDetailedBuildAnalysis(analysis) {
        return JSON.stringify(analysis, null, 2);
    }
    async checkSwiftUIPerformance() {
        // Detectar cálculos costosos en body
        // Validar uso de LazyVStack/LazyHStack
        return [];
    }
    ...
    async checkMemoryUsage(layer) {
        // Analizar retain cycles y [weak self]
        return [];
    }
    generatePerformanceReport(issues) {
        let report = '';
        for (const issue of issues) {
